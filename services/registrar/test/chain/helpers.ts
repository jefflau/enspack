...
/** Hand `enspack-test.eth` to Anvil account 0; account 0 approves account 1 as operator. */
export async function provisionRegistrarRoot(rpcUrl: string): Promise<{
  account0: PrivateKeyAccount;
  account1: PrivateKeyAccount;
  publicResolver: Address;
  publicClient: ForkPublicClient;
  wallet0: ForkWalletClient;
  wallet1: ForkWalletClient;
}> {
  ...
  const approve = await wallet0.writeContract({
    address: ENS_REGISTRY,
    abi: registrarRegistryAbi,
    functionName: "setApprovalForAll",
    args: [account1.address, true],
  });
